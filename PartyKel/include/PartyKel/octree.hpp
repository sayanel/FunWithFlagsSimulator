...
namespace PartyKel
{
    ...
    public:
        ...
        /**
         * Remove all value occurences at a specified position, if it exists
    ...
    void Octree<T>::cleanRecursive(){
        if(DEBUG) DLOG(INFO) << "cleanRecursive at level " << _depth;
        ...
        if(allChildrenEmpty){
            if(DEBUG) DLOG(INFO) << "allChildrenEmpty, clear ";
            _children.clear();
            _initChildren = false;
        }

        if(_madre != nullptr){
            if(DEBUG) DLOG(INFO) << "go to madre at level " << _depth + 1;
            _madre->cleanRecursive();
            return;
        }

        if(DEBUG) DLOG(INFO) << "arrived to root, finished";
    }
